...
def page_correlation_analysis_body():
    ...
    Provides interactive visualizations to explore relationships
    between variables and sale price.
    """
    # load data
    ...
    st.success(
        f"* The client is interested in discovering\
            ...
            Therefore, the client expects data visualisations of the\
    ...
    encoder = OneHotEncoder(variables=df.columns[df.dtypes == 'object']
                            .to_list(), drop_last=False)
    df_ohe = encoder.fit_transform(df)
    # Code copied from "03_correlation_study" notebook -
    # "EDA of chosen variables" section
    ...
    st.info(
        ...
        "The Pearson Correlation measures the strength and"
        "direction of the linear relationship "
        "between two continuous variables."
        "It assesses how well the correlation between the "
        "variables can be represented by a straight line.\n"
        "The heatmap highlights the variables on the x-axis"
        "that have a strong linear "
        "correlation with the Sale Price,"
        "with a correlation coefficient of up to almost 0.8. "
        "The most correlating variables are then visualized"
        "in a bar plot to provide a clearer "
    ...
    st.info(
        ...
        "The Spearman correlation evaluates the monotonic relationship"
        "between two variables, "
        "assessing how well the variables behave similarly"
        "but not necessarily linearly.\n"
        "The heatmap highlights the variables on the x-axis"
        "that have a strong monotonic "
        "correlation with the Sale Price,"
        "with a correlation coefficient of up to almost 0.8. "
        "The most correlating variables are then visualized"
        "in a bar plot to provide a clearer "
    ...
    st.info(
        ...
        "The Predictive Power Score (PPS) detects both linear"
        "and non-linear relationships "
        "between two variables, providing a measure"
        "of their predictive power.\n"
        "The PPS score ranges from 0 (no predictive power)"
        "to 1 (perfect predictive power). "
        "To interpret the plot, locate the 'SalePrice' row on"
        "the y-axis and examine the "
        "variables on the x-axis with a PPS score above 0.2. "
        "Notably, Overall Quality (OverallQual) exhibits the"
        "highest predictive power for the Sale Price target."
        )
...
# function created using "03_correlation_study" notebook code -
# "Distribution of SalePrice" section
